...
# -------------------------
# Normalización de direcciones (mejora precisión)
# -------------------------
def normalize_bogota_address(raw: str) -> str:
    ...
    # Abreviaturas frecuentes Bogotá
    replacements = {
        "CLL": "CALLE",
        "CL": "CALLE",
        "CRA": "CARRERA",
    ...
    # Reemplazo por palabra completa
    for k, v in replacements.items():
        s = re.sub(rf"\b{k}\b", v, s)

    # Normalizar # y espacios
    s = re.sub(r"\s*#\s*", " # ", s)
    s = re.sub(r"\s*-\s*", "-", s)
    s = re.sub(r"\s+", " ", s).strip()
    return s
...
def build_address(row: pd.Series, address_col: str, extra_col: Optional[str]) -> str:
    ...
    # complemento/barrio/torre ayuda
    if extra and extra not in {"NO APLICA", "N/A", "NA"}:
        parts.append(extra)

    # Forzar el contexto a Kennedy / Bogotá
    parts.extend(["LOCALIDAD KENNEDY", "BOGOTÁ", "COLOMBIA"])
    return ", ".join([p for p in parts if p])


# -------------------------
# Geocodificación Google (más precisa)
# -------------------------
...
def geocode_dataframe_within_kennedy(
    ...
    """
    Geocodifica y conserva SOLO puntos que caen dentro del polígono de Kennedy.
    ...
    for i, (idx, row) in enumerate(pending.iterrows(), start=1):
        ...
        # Reintento si cae fuera: variante más corta
        if not inside:
...
# -------------------------
# Mapa (Kennedy + UPZ hover + puntos)
# -------------------------
def make_map(df_points: pd.DataFrame, kennedy_gj: dict, upz_gj: dict) -> folium.Map:
    ...
    # Inyectar BENEFICIARIOS como propiedad para tooltip
    ...
    for _, r in pts.iterrows():
        label = _safe_str(r.get("address_full", "")) or "Dirección"
        upz_name = _safe_str(r.get("UPZ_NOMBRE", "")) or "N/D"
        popup = f"{label}<br><b>UPZ:</b> {upz_name}"
        folium.CircleMarker(
...
# UI
# -------------------------
st.title("Georreferenciar direcciones en Bogotá (solo dentro de Kennedy) + UPZ delimitadas (hover)")

with st.sidebar:
    st.header("Entrada")
    uploaded = st.file_uploader("Sube tu Excel (.xlsx)", type=["xlsx"])

    st.header("Proveedor de geocodificación")
    ...
    run_geocode = st.checkbox("Ejecutar geocodificación ahora", value=False)

    st.header("Salida")
    enable_download = st.checkbox("Permitir descarga del Excel geocodificado", value=True)
...
address_col = st.selectbox("Columna de dirección", options=cols, index=0)
...
if run_geocode:
    ...
    st.info("Activa 'Ejecutar geocodificación ahora' para obtener lat/lon. Solo se conservarán puntos dentro de Kennedy.")
...
st.write(f"Registros: **{total:,}** | Dentro de Kennedy (geocodificados): **{with_coords:,}**")

st.subheader("Mapa: Kennedy (rojo claro) + UPZ delimitadas (hover) + puntos")
m = make_map(df, kennedy_gj, upz_gj)
...
out_cols = [address_col] + ([extra_col] if extra_col else []) + ["address_full", "lat", "lon", "UPZ_CODIGO", "UPZ_NOMBRE"]
...
if enable_download:
    ...
    st.download_button(
        "Descargar Excel con lat/lon y UPZ",
        data=output.getvalue(),
        file_name="direcciones_kennedy_geocodificadas.xlsx",
